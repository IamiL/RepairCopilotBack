...
import (
	"context"
	"log/slog"
	"repairCopilotBot/search-bot/internal/domain/model/chat"
	messagemodel "repairCopilotBot/search-bot/internal/domain/model/message"
	"repairCopilotBot/search-bot/internal/pkg/llmClient"
	"time"
...
type ChatProvider interface {
	...
	ChatShortInfo(ctx context.Context, chatID uuid.UUID) (uuid.UUID, bool, error)
}

type ChatSaver interface {
	CreateChat(ctx context.Context, chatId uuid.UUID, userID uuid.UUID, isProcessing bool, createdAt time.Time, updatedAt time.Time) error
}
