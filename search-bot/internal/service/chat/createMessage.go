...
import (
	...
	"errors"
	"log/slog"
	"repairCopilotBot/search-bot/internal/domain/model/action"
...
func (c *ChatService) NewMessage(ctx context.Context, userID uuid.UUID, chatID uuid.UUID, text string) (string, uuid.UUID, error) {
	...
	if isNewChat {
		...
		err := c.chatSaver.CreateChat(ctx, newChatId, userID, false, now, now)
		if err != nil {
	...
	llmResp, _, err = c.llmClient.Ask(text)
	if err != nil {
	...
	//err = c.chatSaver.UpdateChatTree(ctx, chatID)
	//if err != nil {
